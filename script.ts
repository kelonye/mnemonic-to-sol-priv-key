...
const DerivationPath: { [key: string]: DerivationPath } = {
  Bip44: 'bip44',
...
type DerivationPath = 'bip44' | 'bip44-change' | 'sollet-deprecated';
...
async function main() {
  derive(MNEMONIC, DerivationPath.Bip44Change, 1);
}

function derive(mnemonic: string, dpath: DerivationPath, count: number) {
  const seed = bip39.mnemonicToSeedSync(mnemonic, '');
  for (let i = 0; i < count; i++) {
    for (const kp of deriveSolanaKeypairs(seed, dpath, [i])) {
      console.log(kp.publicKey.toBase58(), base58.encode(kp.secretKey));
    }
  }
}
